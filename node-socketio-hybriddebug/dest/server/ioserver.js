...
var _debugroom = require('./debugroom');

var _debugroom2 = _interopRequireDefault(_debugroom);

var _debugclient = require('./debugclient');

var _debugclient2 = _interopRequireDefault(_debugclient);
...
function ioserverMixmin(io) {
    ...
    function broadcastInRoom(roomId, event, data) {
        ...
        (0, _keys2.default)(clients).forEach(clientId => {
            clients[clientId].socket.emit(event, data);
        });
    }

    io.on('connection', socket => {
        (0, _debug.log)('一个客户端或房间连接');
        ...
        socket.on('create room', (roomId, fn) => {
            ...
            const room = new _debugroom2.default(encodeRoomId, socket);
        ...
        socket.on('create client', (data, fn) => {
            ...
            const client = new _debugclient2.default(encodeRoomId, encodeClientId, socket);
            const assignSocket = socket;
            ...
            (0, _debug.log)(`客户端${clientId}加入房间${roomId}成功`);
        });
        ...
        socket.on('dispatch data', (data, fn) => {
            ...
            (0, _debug.log)(`房间${roomId}分发一条数据:${(0, _stringify2.default)(data)}`);
        ...
        socket.on('disconnect', () => {
            ...
            if (debugRoomsOnline[connectId]) {
                ...
                (0, _debug.log)(`${decodeId(connectId)}房间销毁`);
                ...
                (0, _debug.log)(`${decodeId(connectId)}客户端销毁`);
            }

            (0, _debug.log)('一个客户端或房间失联');
        });
    });
} /**
   * socketio的hybrid调试后台
